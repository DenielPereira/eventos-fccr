...
class ComentarioDAO {
    ...
    public function getComentarioByEvento($id) {
        try {
            $sql = "SELECT eventos_id, comentario, usuario_id, usuario.nome, usuario.sexo FROM comentario
            JOIN usuario ON comentario.usuario_id = usuario.id WHERE eventos_id = '$id'";
